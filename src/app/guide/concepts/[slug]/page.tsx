...
function renderBody(text: string, currentSlug: string) {
  ...
  return paragraphs.map((para, i) => {
    ...
    return (
      <p key={i} className="text-gray-800 dark:text-gray-200 leading-relaxed mb-4">
        {parts.map((part, j) =>
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-sm italic">{concept.subtitle}</p>
      </div>

      {/* Originary definition */}
      <div className="bg-gray-100 dark:bg-gray-800 border-l-4 border-gray-700 dark:border-gray-400 px-5 py-4 rounded-r-xl mb-8">
        <p className="text-xs font-mono text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-2">Originary Definition</p>
        <p className="text-gray-900 dark:text-gray-100 leading-relaxed font-medium">{concept.definition}</p>
      </div>
      ...
      {concept.passages.length > 0 && (
        <section className="mb-10">
          <h2 className="text-xs font-mono text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4">Exemplary Passages</h2>
          <div className="space-y-4">
            {concept.passages.map((p, i) => (
              <blockquote key={i} className="border-l-2 border-amber-300 dark:border-amber-600 pl-4 py-1">
                <p className="text-gray-700 dark:text-gray-300 italic leading-relaxed mb-2">"{p.text}"</p>
                <footer className="text-xs text-gray-500 dark:text-gray-400">
                  —{' '}
                  <Link href={`/post/${p.sourceSlug}`} className="text-blue-600 dark:text-blue-400 hover:underline" target="_blank">
                    {p.source}
                  </Link>
                </footer>
              </blockquote>
      ...
      {concept.selfReference && (
        <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-xl px-5 py-4 mb-10">
          <p className="text-xs font-mono text-amber-700 dark:text-amber-400 uppercase tracking-widest mb-2">Self-Reference</p>
          <p className="text-sm text-amber-900 dark:text-amber-200 leading-relaxed italic">{concept.selfReference}</p>
        </div>
      )}
      ...
      {concept.posts.length > 0 && (
        <section className="mb-10">
          <h2 className="text-xs font-mono text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4">In the Archive</h2>
          <div className="space-y-3">
            {concept.posts.map((post) => (
              <div key={post.slug} className="flex gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-gray-400 dark:bg-gray-600 flex-shrink-0 mt-2" />
                <div>
                  <Link
                    href={`/post/${post.slug}`}
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 pt-6 flex items-center justify-between text-sm">
        ...
        <div className="flex gap-4">
          <Link href="/guide/map" className="text-gray-500 dark:text-gray-400 hover:text-blue-500 transition-colors">View in map</Link>
          <Link href="/guide/reading-paths" className="text-gray-500 dark:text-gray-400 hover:text-blue-500 transition-colors">Reading paths</Link>
        </div>
      </div>
    </main>
  );
